...
TILE_COUNT_PER_WIDTH :: 14
TILE_COUNT_PER_HEIGHT :: 12
...
initilize_chunk_tiles :: proc(world: ^World, chunk: ^WorldChunk) {
	for row, x in &chunk.tiles {
		for tile, y in &row {
			...
			tile.tile_pos = WorldPosition{chunk_pos=chunk.chunk_pos, offset={f32(x), f32(y)}}
		}
	}
...
initilize_world :: proc(world: ^World, buffer_width: i32, buffer_height: i32) {
	...
	world.meters_to_pixels = u32(
		math.min(buffer_height / TILE_COUNT_PER_HEIGHT, buffer_width / TILE_COUNT_PER_WIDTH),
	)
...
update_camera :: proc(game_state: ^GameState, camera_ddp: v2_f32) {
	...
	if !animation.is_active {
		...
		if animation.completed > 100 {
			...
			diff_to_add := chunk_diff / v2_f32{50.0, 50.0}
			game_state.camera_p = map_into_world_pos(
				game_state.world,
				game_state.camera_p,
				diff_to_add,
			)
...
get_chunk_and_tile :: proc(
	...
	half_count :v2_i32 
	half_count.x = (TILE_COUNT_PER_WIDTH/2 )
	half_count.y = (TILE_COUNT_PER_HEIGHT/2 )
	...
	tile = &chunk.tiles[x + half_count.x][y + half_count.y]
	return
}