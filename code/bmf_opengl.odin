...
opengl_draw_rect::proc(min_x:f32, min_y:f32, max_x:f32, max_y:f32, gl_context:^OpenglContext){
	using gl
	Uniform1i(opengl_config.tile_uniform, 1)

	mtop :i32= 100

	if(gl_context.tex_handle == 0){
		opengl_init_texture(gl_context, v2_f32{min_x, min_y}, v2_f32{max_x, max_y}, v4{1,0,0,1}, mtop, mtop)
	}else{
		opengl_update_vertex_data(gl_context, v2_f32{min_x, min_y}, v2_f32{max_x, max_y}, v4{1,0,0,1})
	}
	DrawElements(TRIANGLES, 6, UNSIGNED_INT, rawptr(nil))
