...
opengl_update_vertex_data :: proc(
	gl_context: ^OpenglContext,
	min_p: v2_f32,
	max_p: v2_f32,
	color: v4 = v4{-1, -1, -1, -1},
	...
	if (color.r == -1) {
		...
		BufferSubData(
			ARRAY_BUFFER,
			0 * 9 * size_of(f32) + 5 * size_of(f32),
			4 * size_of(f32),
			&color_mat[0],
		)
		BufferSubData(
			ARRAY_BUFFER,
			1 * 9 * size_of(f32) + 5 * size_of(f32),
			4 * size_of(f32),
			&color_mat[0],
		)
		BufferSubData(
			ARRAY_BUFFER,
			2 * 9 * size_of(f32) + 5 * size_of(f32),
			4 * size_of(f32),
			&color_mat[0],
		)
		BufferSubData(
			ARRAY_BUFFER,
			3 * 9 * size_of(f32) + 5 * size_of(f32),
			4 * size_of(f32),
			&color_mat[0],
		)
	}
